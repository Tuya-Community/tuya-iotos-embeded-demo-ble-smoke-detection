...
static const char auth_key_test[] = "sIahnmVi5anA7GeOx9DSzTjNyb8BfJsW";
static const char device_id_test[] = "tuya376f10f28396";
static const uint8_t mac_test[6] = {0x7D,0x6F,0x31,0x4D,0x23,0xDC}; //The actual MAC address is : 66:55:44:33:22:11 ;DC,23,4D,30,88,A2
...
static void tuya_cb_handler(tuya_ble_cb_evt_param_t* event)
{
    ...
    case TUYA_BLE_CB_EVT_DP_WRITE:
        ...
        TUYA_APP_LOG_HEXDUMP_DEBUG("received dp write",dp_data_array,dp_data_len);
        sn = 0;
...
void app_exe()
{
	if (clock_time_exceed(last_clock, 1000000)) { //1000ms更新
		last_clock = clock_time();

		lux_adc_init();
		adc_value =  adc_sample_and_get_result();
		smoke_sensor_task(adc_value);
